...
//		val directory = documentFile?.findFile(clientDirectoryName)
//		val directoryContent = mutableListOf("..")
...
//				for (file in files) {
//					if (file.isDirectory) {
//						directoryContent.add("${file.name}/")
//					} else {
//						directoryContent.add(file.name)
//					}
//				}
...
//	fun getClientFileSize(filePath: String): Long {
//		val documentFile = DocumentFile.fromTreeUri(context, uri)
//		val file = documentFile?.findFile(filePath)
...
//	}
	...
	fun downloadAndroid(fileName: String, fileContentStream: OutputStream) {
		ftpClient.retrieveFile(fileName, fileContentStream)
	}

	fun uploadAndroid(fileName: String, fileContentStream: InputStream) {
		//сделать загрузку содержимого файла на сервер
//		val targetPath = ftpClient.printWorkingDirectory() + "/" + fileName
...
//	fun createClientDirectory(clientDirectoryUri: Uri) {
//		DocumentFile.fromTreeUri(context, uri)?.createDirectory(clientDirectoryUri.lastPathSegment ?: "")
//	}
...
//	fun removeClientPath(clientFile: DocumentFile?) {
//		clientFile?.delete()
//	}
}